...
# User Deactivate Request View Class
class UserDeactivateRequestView(APIView):
    """
    User Deactivate Request API View Class.
    ...
    # Get Method For Deactivate Request
    @extend_schema(
        operation_id="User Deactivate Request",
        request=None,
        responses={
            status.HTTP_202_ACCEPTED: UserDeactivateRequestAcceptedResponseSerializer,
    ...
    def get(self, request: Request) -> Response:
        """
        Process Deactivate Request For Authenticated User.
        ...
        try:
            ...
            # Build Cache Key
            cache_key: str = f"deactivation_token_{user_id_str}"
            ...
            # If Token Is Invalid
            if not _is_token_valid(cached_token, settings.DEACTIVATION_TOKEN_SECRET):
                # Build Token Payload
                token_payload: dict[str, Any] = {
                    ...
                    "iat": now_dt,
                    "exp": now_dt + datetime.timedelta(seconds=settings.DEACTIVATION_TOKEN_EXPIRY),
                }
            ...
            deactivation_email_template: str = render_to_string(
                ...
                context={
                    ...
                    "deactivation_link_expiry": now_dt + datetime.timedelta(seconds=settings.DEACTIVATION_TOKEN_EXPIRY),
            ...
            return Response(
                data={"message": "Deactivation Request Sent Successfully"},
...
# Exports
__all__: list[str] = ["UserDeactivateRequestView"]
