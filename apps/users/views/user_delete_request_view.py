...
# User Delete Request View Class
class UserDeleteRequestView(APIView):
    """
    User Delete Request API View Class.
    ...
    def get(self, request: Request) -> Response:
        """
        Process Delete Request For Authenticated User.
        ...
        Raises:
            Exception: For Any Unexpected Errors During Delete Request.
        """
        ...
        try:
            ...
            # Build Cache Key
            cache_key: str = f"deletion_token_{user_id_str}"
            ...
            # If Token Is Invalid
            if not _is_token_valid(cached_token, settings.DELETION_TOKEN_SECRET):
                # Build Token Payload
                token_payload: dict[str, Any] = {
                    ...
                    "iat": now_dt,
                    "exp": now_dt + datetime.timedelta(seconds=settings.DELETION_TOKEN_EXPIRY),
                }
                ...
                # Cache New Token
                token_cache.set(cache_key, new_token, timeout=settings.DELETION_TOKEN_EXPIRY)
            ...
            deletion_link: str = f"{protocol}://{current_site.domain}/api/users/delete/confirm/{cached_token}/"
            ...
            deletion_email_template: str = render_to_string(
                ...
                context={
                    ...
                    "deletion_link_expiry": now_dt + datetime.timedelta(seconds=settings.DELETION_TOKEN_EXPIRY),
            ...
            try:
                ...
                record_email_sent(email_type="deletion_request", success=True)

            except Exception:
                # Record Email Sent Failure
                record_email_sent(email_type="deletion_request", success=False)

                # Raise Exception
                raise

            # Record Delete Request Initiated
            record_delete_request_initiated()

            # Record HTTP Request Metrics For 202
            duration_202: float = time.perf_counter() - start_time
            record_user_action(action_type="delete_request", success=True)
            record_http_request(
            ...
            return Response(
                data={"message": "Deletion Request Sent Successfully"},
            ...
            record_user_action(action_type="delete_request", success=False)
...
# Exports
__all__: list[str] = ["UserDeleteRequestView"]
