...
# User Email Change Request View Class
class UserEmailChangeRequestView(APIView):
    """
    User Email Change Request API View Class.
    ...
    # Get Method For Email Change Request
    @extend_schema(
    ...
    def get(self, request: Request) -> Response:
        """
        Process Email Change Request For Authenticated User.
        ...
        try:
            ...
            # Build Cache Key
            cache_key: str = f"change_email_token_{user_id_str}"
            ...
            # If Token Is Invalid
            if not _is_token_valid(cached_token, settings.CHANGE_EMAIL_TOKEN_SECRET):
                # Build Token Payload
                token_payload: dict[str, Any] = {
                    ...
                    "iat": now_dt,
                    "exp": now_dt + datetime.timedelta(seconds=settings.CHANGE_EMAIL_TOKEN_EXPIRY),
                }
            ...
            email_change_link: str = (
                f"{protocol}://{current_site.domain}/api/users/change-email/confirm/{cached_token}/"
            )

            # Load Email Change Email Template
            email_change_email_template: str = render_to_string(
                template_name="users/user_email_change_request.html",
                context={
                    "first_name": user.first_name,
                    "last_name": user.last_name,
                    "email": user.email,
                    "email_change_link": email_change_link,
                    "email_change_link_expiry": now_dt + datetime.timedelta(seconds=settings.CHANGE_EMAIL_TOKEN_EXPIRY),
            ...
            return Response(
                data={"message": "Email Change Request Sent Successfully"},
...
# Exports
__all__: list[str] = ["UserEmailChangeRequestView"]
